...
static void push_quad(ktk_Rect dst, ktk_Rect src, ktk_Color color) {
...
void r_draw_text(const char *text, ktk_Vec2 pos, ktk_Color color) {
  ktk_Rect dst = { pos.x, pos.y, 0, 0 };
  for (const char *p = text; *p; p++) {
    if ((*p & 0xc0) == 0x80) { continue; }
    int chr = ktk_min((unsigned char) *p, 127);
    ktk_Rect src = atlas[ATLAS_FONT + chr];
    dst.w = src.w;
    dst.h = src.h;
...
void r_draw_icon(int id, ktk_Rect rect, ktk_Color color) {
  ktk_Rect src = atlas[id];
  int x = rect.x + (rect.w - src.w) / 2;
  int y = rect.y + (rect.h - src.h) / 2;
  push_quad(ktk_rect(x, y, src.w, src.h), src, color);
}
